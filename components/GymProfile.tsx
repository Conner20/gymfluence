          ...
          <ul className="mt-4 space-y-3">
            ...
            {history.map((r) => (
              <li key={r.id} className="border rounded p-3 dark:border-white/10">
                ...
                {r.comment && (
                  <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">{r.comment}</p>
                )}
              </li>
            ))}
          </ul>
      ...
      >
        <div className="w-full px-6 flex flex-col items-center gap-4">
          ...
          <div className="w-full mt-2 rounded-2xl border border-gray-200 px-4 py-3 space-y-3 dark:border-white/10">
            ...
            <ProfileStat
              label="monthly fee"
              value={typeof gym?.fee === "number" ? `$${gym.fee.toFixed(2)}/mo` : "N/A"}
            />
            ...
            <div className="pt-1 border-t border-gray-100 dark:border-white/10">
              <div className="flex items-center justify-between text-center text-xs text-gray-500 dark:text-gray-400">
                <button
                  ...
                  className={clsx(
                    "flex-1 flex flex-col py-1 text-center items-center rounded-md",
                    canViewPrivate
        ...
        {!canViewPrivate ? (
          ...
          <>
            <div className="mb-4 flex items-center justify-between bg-[#f8f8f8] py-2 lg:py-0 lg:sticky lg:top-0 lg:z-10 dark:bg-[#050505]">
              ...
              <div className="inline-flex rounded-full border bg-white p-1 dark:bg-transparent dark:border-white/10">
                <button
                  className={clsx(
                    "px-3 py-1 text-sm rounded-full",
                    viewMode === "grid"
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      {posts.map((post) => (
        <button
          key={post.id}
          className="bg-white rounded-lg flex items-center justify-center w-full h-56 overflow-hidden relative border border-zinc-200 transition hover:shadow-lg dark:bg-neutral-900 dark:border-white/10"
