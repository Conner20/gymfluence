...
/** Manage modal (trainer) — shows rater name for PENDING, reveals stars/comment after APPROVE, X to dismiss approved, removes on DECLINE */
function ManageTrainerRatingsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    React.useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                const rows: PendingRating[] = await fetch("/api/ratings?pendingFor=trainer", { cache: "no-store" }).then((r) => r.json());
    ...
    async function approve(id: string) {
        try {
            ...
            // Try to fetch full details after approval (optional if your backend returns them already)
                            ...
                            <li key={r.id} className="border rounded p-3 relative">
                                ...
                                <div className="flex items-center justify-between pr-6">
                                    <div className="font-medium">
                                        ...
                                        {r.status === "PENDING" ? "left a rating" : "rated you"}
                                    </div>
    ...
    return (
        <div className="flex min-h-screen">
            <aside className="w-72 bg-white flex flex-col items-center pt-8">
                ...
                {!isOwnProfile && (
                    <>
                        {/* Row 1: Follow/Unfollow on its own full-width line (prevents overflow) */}
            ...
            <ManageTrainerRatingsModal open={showManageRatings} onClose={() => setShowManageRatings(false)} />
        </div>
    );
}
