...
function ManageTrainerRatingsModal({
    open,
    onClose,
    onApproved, // update sidebar rating/clients immediately
}: {
    open: boolean;
    onClose: () => void;
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                if (tab === "pending") {
                    const rows: PendingLite[] = await fetch("/api/ratings?pendingFor=trainer", { cache: "no-store" }).then((r) => r.json());
                    setPending(rows || []);
                } else {
                    const rows: HistoryRow[] = await fetch("/api/ratings?historyFor=trainer", { cache: "no-store" }).then((r) => r.json());
                    setHistory(rows || []);
                }
            } catch (e: any) {
    ...
    async function act(id: string, action: "APPROVE" | "DECLINE") {
        try {
            ...
            try {
                ...
                // Fallback if GET fails: just switch to history and refetch
                setPending((prev) => prev.filter((r) => r.id !== id));
                setTab("history");
                const rows: HistoryRow[] = await fetch("/api/ratings?historyFor=trainer", { cache: "no-store" }).then((r) => r.json());
                setHistory(rows || []);
...
export function TrainerProfile({ user, posts }: { user: any; posts?: BasicPost[] }) {
    ...
    return (
        <div className="flex min-h-screen">
                ...
                <div className="flex justify-center items-center mb-3">
                    {user.image ? (
                        ...
                        <div className="w-24 h-24 rounded-full bg-white border border-gray-200 flex items-center justify-center">
                            <span className="text-green-700 font-bold text-xl select-none text-center px-2 break-words">{user.username || user.name || "User"}</span>
                        </div>
                    )}
                </div>
                    ...
                    <>
                        <div className="flex flex-wrap items-center gap-2 mb-4 justify-center">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setShowRate(true)} className="flex items-center gap-2 px-3 py-1.5 rounded-full border text-sm bg-white hover:bg-[#f8f8f8] transition" title="Rate this trainer">
                                    <Star size={16} />  
                                </button>
                            </div>
                        </div>
            ...
            <RateTrainerModal open={showRate} onClose={() => setShowRate(false)} trainerId={trainer?.id} />
